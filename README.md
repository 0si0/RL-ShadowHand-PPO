...
Isaac Lab 환경에서 Shadow Hand가 물체를 조작하도록 PPO 정책을 학습시킨 강화학습 프로젝트입니다. HO-Cap 기반 human hand/object trajectory를 reference로 사용하고, 관측값, 보상 함수, curriculum, reset strategy, TensorBoard metric을 직접 설계하여 hand imitation과 object manipulation을 함께 최적화했습니다.
...
| 핵심 목표 | 사람 손동작과 물체 trajectory를 동시에 추종하는 정책 학습 |
...
이 과제는 단순히 PPO를 실행하는 문제가 아니라, 물리 시뮬레이션에서 로봇 손이 실제로 물체에 힘을 전달하도록 학습 신호를 설계하는 문제였습니다. Shadow Hand와 human hand는 관절 구조가 다르기 때문에, 사람 손 keypoint를 그대로 따라가는 것만으로는 자연스러운 robot joint posture나 stable contact가 만들어지지 않았습니다.

그래서 정책이 학습 중에 다음 네 가지 정보를 동시에 보도록 환경을 재설계했습니다.
...
`Reinforcement Learning` `PPO` `Isaac Lab` `Robot Manipulation` `Shadow Hand` `Reward Shaping` `Curriculum Learning` `Imitation Learning` `TensorBoard` `PyTorch`
